...
export default function AuditScreen() {
  ...
  // 3 mock asset rows with initial status 'pending'
  ...
  const flaggedAssets = assets.filter(asset => asset.status === 'missing' || asset.status === 'damaged');
          ...
          <tbody className="divide-y divide-slate-100">
            {assets.map((asset) => (
              <tr key={asset.id} className="hover:bg-slate-50/50 transition-colors">
                <td className="px-4 py-4 font-mono font-medium text-slate-900">{asset.id}</td>
                <td className="px-4 py-4 text-slate-700">{asset.name}</td>
                <td className="px-4 py-4 text-slate-500">{asset.expectedLocation}</td>
